-- V3 sales RPC update:
-- 1) Auto-link/create customer in same transaction when customer_phone is present.
-- 2) Deduct product components from inventory_items.current_stock when configured.
-- Backward compatible: if no product_components rows exist for a sold product, no component deduction is attempted.
